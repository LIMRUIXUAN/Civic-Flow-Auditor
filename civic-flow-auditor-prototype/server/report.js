    ...
    .map((finding) => {
      ...
      return `
        <article class="finding">
          ...
          ${dataUrl ? `<img src="${dataUrl}" alt="Annotated screenshot evidence for ${escapeHtml(finding.id)}">` : ""}
          ${evidenceLink}
...
<body>
  <main>
    ...
    <p><b>Website:</b> ${escapeHtml(auditRun.url)}<br><b>Scan depth:</b> ${escapeHtml(auditRun.depth)}<br><b>Status:</b> ${escapeHtml(auditRun.status)}</p>
    <p class="meta"><b>Report mode:</b> ${escapeHtml(aiLabel)}<br><b>Lighthouse accessibility:</b> ${escapeHtml(lighthouseLabel)}<br><b>OCR:</b> ${escapeHtml(auditRun.scanner?.ocr?.status || "not-run")}</p>
    ...
    <h2>Documents And Handbooks</h2>
    <table>
      <thead><tr><th>Document</th><th>Extracted text length</th><th>Status</th><th>Summary</th></tr></thead>
      <tbody>${documentRows || "<tr><td colspan=\"4\">No linked PDFs were found.</td></tr>"}</tbody>
    </table>
